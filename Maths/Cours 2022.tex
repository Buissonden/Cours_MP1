...
\par\begin{center}$E(X)=\sum\limits_{n\in\N}P(X\geq n)$\end{center}}
...
\Thr{Espérace d'un produit de variables indépendantes}{Soit $X,Y$ deux variables aléatoires discrètes à valeurs réelles ou complexes.
\par Si on a :\begin{itemize}
\item $X$ et $Y$ d'espérance finie
\item $X$ et $Y$ indépendantes
\end{itemize}
Alors $XY$ est d'espérance finie et \par\begin{center}$E(XY)=E(X)E(Y)$\end{center}}
\Thr{Espérace d'un produit de n variables indépendantes}{Soit $X_1,...,X_n$ des variables aléatoires discrètes à valeurs réelles ou complexes.
\par Si on a :\begin{itemize}
...
\par Si de plus les variables $X_1,...,X_n$ sont indépendantes deux à deux alors :
