...
\section{Hors programme}
\subsection{Fonctions k fois différentiables}
\Def{}{Soit $f:U\subset E\to F$
\par $f$ est $k$ fois différentiable pour $k\geq 1$ si $f$ est $(k-1)$ fois différentiable et $d^{k-1}f$ est différentiable.
\par $f$ est $\mathcal{C}^k$ si $f$ est $k$ fois différentiable et $d^kf$ est continue.}
\begin{Exe}
Si $f$ deux fois différentiable, pour $x\in U, h,k\in E$
\par $$d^2f(x)\in\mathcal{L}(E,\mathcal{L}(E,F))$$
\par $$d^2f(x)\cdot H\in\mathcal{L}(E,F)$$
\par $$d^2f(x)\cdot h\cdot k = (d^2f(x)\cdot h)\cdot k\in F$$
\par Par théorème de Schwarz, si $f$ est $2$ fois différentiable alors $\forall h, k\in E, d^2f(x)\cdot h\cdot k = d^2f(x)\cdot k\cdot h$
\par Si $(e_1,..., e_n)$ est une base de $E$
\par On sait que $\partial_jf(x) = df(x)\cdot e_j$
\par Et donc $\partial_i\partial_j f(x) = (t\mapsto df(x+te_i)\cdot e_j)'(0) = d^2f(x)\cdot e_i\cdot e_j$
\par En effet, $df(x+te_i)\cdot e_j = (df(x)+ d^2 f(x)\cdot e_i+o(t)) = df(x)\cdot e_j + td^2f(x)\cdot e_i\cdot e_j+o(t)$
\par Si $h = h_1e_1+...+h_ne_n$ et $k=k_1e_1+...+k_ne_n$
\par \begin{align*}d^2f(x)\cdot h\cdot k &= d^2f(x)\cdot\left(\sum\limits_{i=1}h_ie_i\right)\cdot\left(\sum\limits_{j=1}^nh_je_j\right)\\
&=\sum\limits_{1\leq i, j\leq n}h_ik_jd^2f(x)\dot e_i\cdot e_j
&= \sum\limits_{1\leq i, j\leq n}h_ik_j\partial_i\partial_jf(x)
\end{align*}
\par Si $F = \R$
\par $d^2f(x)\cdot h\cdot k = H^TH_f(x)K$ avec $H = Mat_\mathcal{B}(h)$ et $K = Mat_\mathcal{B}(k)$
\par La formule de Taylor à l'ordre $k$ devient :
\par Avec $U$ un ouvert connexe, $a,b\in U$.
\par Si $f$ est une fonction $\mathcal{C}^{k+1}$
\par $\varphi:\left\{\begin{array}{rcl} [0,1] & \to & F \\ t & \mapsto & f(a+(b-a)t)\end{array}\right.$
\par Alors $\varphi$ est $\mathcal{C}^{k+1}$
\par \begin{align*}\forall t\in[0,1], &\varphi'(t) = df(a+t(b-a))\cdot(b-a)\\
&\varphi''(t) = d^2f(a+t(b-a))\cdot(b-a)\cdot(b-a)
&...
&\varphi^p(t) = d^pf(a+t(b-a))\cdot (b-a)^p
\end{align*}
\par Et donc $\varphi(1) =\varphi(0)+\varphi'(0)+\frac{1}{2}\varphi''(0) +...+\varphi^{(k)}(0) + \int_0^1\frac{(1-t)^k}{k!}d^{k+1}f(a+t(b-a))\cdot (b-a)^{k+1}dt$
\par Ce qui nous donne finalement :
\par $$f(b) = f(a) + df(a)\cdot (b-a)+\frac{1}{2}d^2f(a)\cdot (b-a)\cdot(b-a)+...+\frac{1}{k!}d^kf(a)\cdot (b-a)^k + o(h)$$
\par Donc pour revenir au premier ordre $f(a+h) = f(a)+df(a)\cdot h + \int_0^1(1-t)d^2f(a+h)\cdot h\cdot hdt$
\end{Exe}

\subsection{Démo hors-programme : fonction constante différentiable}
\Def{}{Soit $U$ un ouvert de $E$ connexe par arc.
\par Si $f:U\to F$ est différentiable, alors :
\par $$f\text{ constante }\Leftrightarrow \forall x\in U, df(x)=0$$}
\Pre{On a déjà prouvé le sens direct, et le sens indirect quand $U$ était un ouvert convexe.
\par On suppose $U$ connexe par arcs non-vide et que $\forall x\in U, df(x)=0$.
\par On choisit $x_0\in U$
\par On considère $A =\{x\in U\vert f(x)=f(x_0)\}$
\par On va montrer que $A = U$
\par $A = f^{-1}(\{f(x_0)\})$, or $f$ est continue, donc $A$ est l'image réciproque du fermé $\{f(x_0)\}$ par une application continue.
\par Donc $A$ est un fermé relatif de $U$.
\par Soit $x\in A$, comme $U$ est ouvert on prend $\alpha>0$ telle que $\mathcal{B}(x,\alpha)\subset U$
\par Or $\mathcal{B}(x, \alpha)$ est convexe
\par Donc $\forall y\in B(x, \alpha), f(y)-f(x) = \int_0^1df(x+t(y-x))\cdot (y-x)dt=0$
\par Donc $\mathcal{B}(x,\alpha)\subset A$
\par Donc $A$ est un ouvert.
\par Dans un ouvert connexe par arcs, un ensemble qui est un ouvert et un fermé en même temps est soit vide, soit l'ensemble tout entier.
\par Pour le montrer, supposons par l'absurde que $\mathcal{C}_UA\neq \emptyset$
\par On a donc $y_0\in\mathcal{C}_UA$
\par Comme $U$ est connexe par arcs, on a une arc continu de $x_0$ à $y_0$ :
\par $$\gamma\left\{\begin{array}{rcl}[0,1] & \to & U \\ t & \mapsto & \gamma(t)\end{array}\right.$$
\par où $\gamma(0)=x_0\in A$ et $\gamma(1) =y_0\in\mathcal{C}_UA$
\par On peut donc considérer $t_0\in[0,1]$ tel que $t_0 = \sup\{t\in[0,1]\vert \gamma(t)\in A\}$
\par $\mathcal{C}_UA$ est ouvert donc $t_0<1$ ($\exists\beta\in\R_+^*, \mathcal{B}(y_0,\beta)\subset\mathcal{C}_UA$)
\par Et $\gamma(t_0)\in A$ : il existe $t_n\in[0,1]^\N$ tel que $t_n\to t_0$ et $\forall n\in\N,\gamma(t_n)\in A$ par caractérisation séquentielle du sup ; donc par continuité de $f\circ\gamma$, $f\circ\gamma(t_0)=f(x_0)$
\par Or $A$ est ouvert, donc il existe $\delta\in\R_+^*$ tel que $\mathcal{B}(\gamma(t_0), \delta)\subset A$
\par Ce qui nie la définition de $t_0$
\par D'où la contradiction.}

\subsection{Différentielle d'un inverse}
\Prop{Dans R}{Si $f$ dérivable bijective d'un intervalle $I$ sur un intervalle $J$, si $f'$ ne s'annule pas alors $f^{-1}$ est dérivable et :
\par $$(f^{-1})' = \dfrac{1}{f'\circ f^{-1}}$$}
\Prop{Dans le cas général}{Si $f$ est différentiable bijective d'un ouvert $U$ sur un ouvert $V$, et si $f^{-1}$ est différentiable
...
\par $$df^{-1}(f(x)) = (df(x))^{-1}$$}
\begin{Exe}
$$\varphi:\left\{\begin{array}{rcl} \R_+^*\times ]0, 2\pi[ & \to & \R^2\backslash D_+ \\ (r,\theta) & \mapsto & (x,y) = (r\cos\theta, r\sin\theta)\end{array}\right.$$
\par Avec $D_+$ la demi-droite $\{(x,y)\in\R^2\vert y=0, x\geq0\}$. C'est le seul moyen d'avoir une application en polaire qui soit bijective.
\par $$J_\varphi(r,\theta) = \begin{pmatrix}\cos\theta & -r\sin\theta \\\ \sin\theta & r\cos\theta\end{pmatrix}$$
\par Et $J_{\varphi^{-1}}(r\cos\theta,r\sin\theta) = \begin{pmatrix} \cos\theta & \sin\theta \\ \frac{-\sin\theta}{r} & \frac{\cos\theta}{r}\end{pmatrix}$
\par $$\psi:\left\{\begin{array}{rcl} \R_+^*\times ]0,2\pi[\times ]0,2\pi[ &\to & \R^3\backslash P^+ \\ (r,\theta,\varphi) & \mapsto & (x,y,z) = (r\sin\theta\cos\varphi, r\sin\theta\sin\varphi, r\cos\theta)\end{array}\right.$$
\par Qui est bijective à l'exception d'un demi-plan.
\end{Exe}

\chapter{Groupes et anneaux}
\section{Rappels de sup}
...
\Prop{Unicité du neutre}{Il n'y a qu'un seul élément neutre dans un groupe}
\Pre{Si $e$ et $e'$ étaient des neutres, alors $e\star e' = e = e'$. D'où l'unicité.}
...
\Prop{Unicité du symétrique}{Si la lci est associative, alors $x$ admet un unique symétrique}
\Pre{Supposons que $x\in E$ admette deux symétriques, $x'$ et $x''$.
\par \begin{align*}x'\star x\star x'' &= (x'\star x)\star x''
&= x''
&= x'\star(x\star x'')
&=x'
\end{align*}}
\Def{Groupe}{Un groupe est un ensemble qui est muni d'une lci associative, qui possède un neutre et où tout élément admet un symétrique.
\par Un groupe commutatif ou abélien esr un groupe où la lci est commutative.
\par Un ensemble muni d'une lci associative avec un neutre mais où tout élément n'est pas symétrisable est appelé un monoïde.}
\Prop{}{Si $G$ un monoïde si $a,b\in G$ sont symétrisables, alors $ab$ est symétrisable et $(ab)^{-1} = b^{-1}a^{-1}$}
\Pre{\begin{align*} abb^{-1}a^{-1} &= aea^{-1}\\
&= aa^{-1}
&= e
\end{align*}
\par Donc on a exhibé un inverse de $ab$, qui est unique.}
...
\end{cases}$}
\begin{Rem}
Notations : en général, on les prend de manières multiplicatives ou additives. On réserve les notation additives pour les groupes commutatifs la plupart du temps.
\par On note $\star$ $\cdot$ de manière multiplicative. On note $1$ le neutre le plus souvent.
\par Pour $a\in G$, on note : $\forall n\in\N^*, a^n = a\cdot...\cdot a$ n fois.
\par Par convention, on a $a^0 = 1$
\par Pour $n\in\N^*$, $a^{-n} = a^{-1}\star...\star a^{-1}$ n fois.
\par On note $\langle a\rangle = \{a^n\vert n\in\Z\}$ le groupe engendré par $a$. C'est le plus petit groupe contenant $a$.
\par On note $\star$ $\intercal$ de manière additive. On note $0$ le neutre le plus souvent, et $-a$ le symétrique.
\par Pour $a\in G$, on note : $\forall n\in\N^*, na = a\intercal...\intercal a$ n fois.
\par Par convention, on a $0a = 0$
\par Pour $n\in\N^*$, $-na = -a\star...\star -a$ n fois.
\par On note $\langle a\rangle = \{na\vert n\in\Z\}$ le groupe engendré par $a$. C'est le plus petit groupe contenant $a$.
\end{Rem}
\begin{Exe}
Exemples fondamentaux :
$(\Z, +)$ est un groupe, mais pas $\N$
\par Si $G$ un groupe et $A$ un ensemble quelconque, alors $\mathcal{F}(A,G)$ est un groupe pour la loi $(f,g)\mapsto fg =(a\mapsto f(a)\star g(a))$
\par Avec $(G_1,\star_1), (G_2, \star_2)$ deux groupes, alors $G_1\times G_2$ est un groupe, muni de la loi $(g_1, g_2)\star (g_1', g_2') = (g_1\star_1 g_1', g_2\star_2 g_2')$  
\par Par exemple, $(\Z, +)\times (\R_+^*, \cdot)$ est un groupe et $(k,x)\cdot (l, y) = (k+l, xy)$ est sa lci.
\par Avec $A$ un ensemble, $\mathcal{B}(A,A)$ l'ensemble des bijections de $A$ dans $A$, aussi noté $S_A (=\mathfrak{S}(A))$ quand $A$ est fini, est un groupe pour la composition.
\end{Exe}
\Def{}{Avec $(G,\star)$ un groupe et $\mathcal{R}$ une relation d'équivalence sur $G$
\par On note $\mathcal{G}/\mathcal{R} = \{\overline{g_\mathcal{R}}\vert g\in G\}$ où $\overline{g_\mathcal{R}} = \{g'\in G\vert g'\mathcal{R}g\}$
\par Si $\mathcal{R}$ est compatible avec $\star$ alors on peut définir une lci naturelle sur $G_\mathcal{R}$
\par $$\cdot:\left\{\begin{array}{rcl} G_\mathcal{R}\times G_\mathcal{R} & \to & G_\mathcal{R} \\ \overline{x}, \overline{y} &\mapsto & \overline{x\star y}\end{array}\right.$$
\par Et dans ce cas $\varphi:\left\{\begin{array}{rcl} G & \to & G_\mathcal{R} \\ x & \mapsto & \overline{x}\end{array}\right.$ est un morphisme surjectif, qu'on appelle la projection.}
\begin{Exe}
Dans $(\Z, +)$, avec $n\in\N, n\geq 2$, on note $\mathcal{R} :\equiv n$ la congruence modulo n définie par :
\par $$x\mathcal{R} y \leftrightarrow x-y\equiv 0 [n]\Leftrightarrow (x-y)\in n\Z$$
\par On appelle ce groupe le groupe quotient.
\par On peut généraliser cette notion : Si $G$ un groupe, et $H$ un sous-groupe de $G$, on définit $\mathcal{R}_H$ sur $G$ par :
\par $$\forall x,y\in G, x\mathcal{R}_H y \Leftrightarrow xy^{-1}\in H$$
\par C'est bien une relation d'équivalence :
\par $\forall x\in G, xx^{-1} = 1$ et $H$ est un sous-groupe de $G$ donc $1\in H$ donc $\mathcal{R}_H$ est réflexive.
\par $\forall x,y\in G, xy^{-1}\in H$ donne que $(xy^{-1})^{-1}\in H$, or $(xy^{-1})^{-1}=yx^{-1}$, donc $y\mathcal{R}_Hx$, donc $\mathcal{R}_H$ est symétrique.
\par $\forall x,y,z\in G, x\mathcal{R}_Hy$ et $y\mathcal{R}_Hz$ donne que $xy^{-1}\in H$ et $yz^{-1}\in H$, donc $xy^{-1}=xy^{-1}yz^{-1}\in H$, donc $\mathcal{R}_H$ est transitive.
\par Malheureusement, cette relation n'est pas toujours compatible. C'est vrai si la loi est commutative.
\par On peut quand même avoir ce résultat :
\par Soit $y\in G$, soit $x\in G$.
\par \begin{align*} x\in \overline{y_{\mathcal{R}_H}} & \Leftrightarrow xy^{-1}\in H\\
&\Leftrightarrow \exists h\in H, xy^{-1}=h\\
&\Leftrightarrow \exists h\in H, x = hy\\
&\Leftrightarrow x\in Hy
\end{align*}
\par Et donc $\overline{y_{\mathcal{R}_H}} = Hy = \{hy\vert h\in H\}$
\par Donc le cardinal d'une classe selon $\mathcal{R}_H$ est le cardinal de $H$.
\par Donc le cardinal de $G$ est le cardinal de $H$ multiplié par le nombre de classes.
\par On peut en déduire le théorème (hors-programme) de Lagrange :
\par Si $G$ est un groupe fini et $H$ un sous-groupe de $G$, alors $\mathrm{card} H\vert\mathrm{card} G$
\par On le prouve en considérant $\mathcal{R}_H : x\mathcal{R}_Hy \Leftrightarrow xy^{-1}\in H$
\par Et donc $\forall y\in G, \mathrm{card} \overline{y_{\mathcal{R}_H}}=\mathrm{card} H$
\par Donc $\mathrm{card} G = \mathrm{card}(G/\mathcal{R}_H)\mathrm{card} H$
\par Revenons sur la condition de compatibilité de $\mathcal{R}_H$ :
\par $$\forall (x,y)\in G^2, HxHy = \{h_1xh_2y \vert (h_1,h_2)\in H^2\}\subset Hxy$$
\par L'autre inclusion est toujours vraie : $\forall x,y\in G, \forall h\in H, hx1y= hxy\in Hxy$ et $1\in H$ car $H$ est un sous-groupe de $G$
\par Si le groupe est commutatif, alors : $\forall x,y\in G, \forall h_1, h_2\in H, h_1xh_2y = (h_1h_2)xy$ et $h_1h_2\in H$ car $H$ est un sous-groupe de $G$. 
\par Les sous-groupes qui fonctionnent sont appelés les groupes distingués.
\end{Exe}


\section{Groupes engendrés par une partie non-vide}
\Prop{Intersection}{Si $G$ un groupe
\par Si $(H_i)_{i\in I}$ une famille de sous-groupes de $G$
\par Alors $(\cap H_i)_i\in I$ est un sous-groupe de $E$}
\Pre{$\forall i\in I, e\in H_i$ donc $e\in (\cap H_i)_{i\in I}$
\par Si $x,y\in G$ tels que $\forall i\in I, x,y\in H_i$ alors $\forall i\in I, xy\in H_i$ comme c'est un sous-groupe donc $xy\in (\cap H_i)_{i\in I}$
\par Donc c'est bien un sous-groupe de $G$.}
\begin{Rem}
On peut donc définir un groupe engendré par un élément $a$ comme l'intersection de tous les sous-groupes de $G$ contenant $a$.
\end{Rem}
\Def{}{Si $A\neq\emptyset$, alors le plus petit sous-groupe de $G$ contenant $A$ est l'intersection de tous les sous-groupes de $G$ contenant $H$.
\par C'est le groupe engendré par $A$, noté $\langle A\rangle$}
\Def{Groupe monogène}{On note $\langle \{a\} \rangle = \langle a\rangle =\{a^n\vert n\in\Z\}$}
\Pre{On prouve que c'est un groupe.
\par Ensuite, on prouve que tout groupe contenant $a$ contient aussi les itérés de $a$.}
\Def{}{Si $a,b\in G$ :
\par Si $G$ est commutatif, alors $\langle\{a,b\rangle = \{a^pb^q\vert (p,q)\in\Z^2\}$
\par Sinon, on a $\langle\{a,b\}\rangle= \{w\in G\vert \exists n\in\N, w=x_1...x_n\text{ et } \forall i\in\llbracket 1,n\rrbracket, x_i\in\{a,b,a^{-1}, b^{-1}\}\}$}
\begin{Rem}
Ainsi, on ne traitera pas les groupes engendrés par plus d'un élément, puisqu'il est très difficile de les manipuler.
\end{Rem}

\section{Groupes monogènes}
...
\begin{Rem}
$(\Z,+)$ est monogène
\par $\Z/n\Z$ est cyclique
\par $(\mathbb{U}_n,\times)$ dans $(\C^*,\times)$ est cyclique
\end{Rem}
...
\Prop{}{Si $\varphi$ est un morphisme de $G$ dans $G'$ :\begin{itemize}
...
\end{itemize}}
\Pre{Pour la première, $\varphi(ee) = \varphi(e)\varphi(e) = \varphi(e)$
\par Donc : $(\varphi(e))^{-1}\varphi(e)\varphi(e) = (\varphi(e))^{-1}\varphi(e) = e'$
\par Pour l'image directe : $\varphi(H) = \{y\in G', \exists x\in H, \varphi(x)=y\}$
\par $e\in H$ donc $\varphi(e)\in\varphi(H)$
\par Si $x', y'\in\varphi(H)$, on a donc $x,y\in H$ tels que $\varphi(x) = x'$ et $\varphi(y)=y'$
\par Donc $x'y'=\varphi(x)\varphi(y)=\varphi(xy)\in\varphi(H)$
\par Et on fait de même pour les inverses.
\par Pour l'image réciproque : on prend $\varphi^{-1} = \{x\in G\vert \varphi(x)\in H'\}$ et on fait le même raisonnement.}
\begin{Rem}
À ne pas confondre avec le cas d'un morphisme de lci, qui, si on a $E$ et $E'$ munis de lci, est une application $\varphi$ de $\mathcal{F}(E,E')$
\par Si $\varphi$ est un morphisme de lci \underline{surjectif}, alors $\varphi$ transporte les propriétés de $E$ dans $E'$ :
\par Si $E$ possède un neutre $e$ :
\par Soit $x'\in E'$
\par Par surjectivité, on a $x\in E$ tel que $\varphi(x)=x$
\par $\varphi(e)x'=\varphi(e)\varphi(x)=\varphi(ex) =\varphi(x)=x'$
\par Donc $\varphi(e)$ est le neutre de $E'$.
\par La commutativité ou l'associativité de la lci sur $E$ et l'existence de symétriques est héritée sur un morphisme de lci surjectif.
\end{Rem}
\Def{Sous-groupes particuliers}{Si $\varphi$ est un morphisme de $G$ dans $G'$,
\par On note $Im(\varphi) = \varphi(G)$ et $\ker\varphi = \varphi^{-1}(\{e'\})$}
\Prop{}{$\varphi$ est injective si, et seulement si, $\ker\varphi = \{e\}$}
\Pre{Pour $x,y\in G$
\par \begin{align*} \varphi(x)=\varphi(y) &\Leftrightarrow \varphi(x)\varphi(y)^{-1} &= e'\\
&\Leftrightarrow \varphi(xy^{-1}) &= e'\\
&\Leftrightarrow xy^{-1}\in\ker\varphi
\end{align*}
\par Si $\varphi$ est injectif, alors $\varphi(x) =\varphi(y) \Rightarrow x=y$, donc $\ker\varphi =\{xx^{-1}\vert x\in G\}=\{e\}$
\par Si $\ker\varphi= \{e\}$ : pour $x,y\in G$ tels que $\varphi(x)=\varphi(y)$, alors $xy^{-1} = e\Leftrightarrow x=y$
\par Donc $\varphi$ est injectif.}
...
\end{itemize}}
\Pre{Soit $G$ un groupe monogène, $G = \{a^n\vert n\in\Z\}$.
\par on considère $\varphi:\left\{\begin{array}{rcl}\Z&\to& G\\ n & \mapsto & a^n\end{array}\right.$
\par $\varphi$ est un morphisme du groupe $(\Z,+)$ dans $(G,\times)$. Le morphisme est surjectif.
\par $\ker\varphi$ est un sous-groupe de $\Z$
\par Donc il existe $n_0\in\N$ tel que $\ker\varphi = n_0\Z$
\par Si $n_0 = 0$, alors $\varphi$ est homogène donc $G$ est isomorphe à $\Z$
\par Si $n_0\geq 1$, considérons :
\par $$\overline{\varphi}:\left\{\begin{array}{rcl} \Z/n_0\Z & \to & G \\ \overline{p} & \mapsto & \varphi(p)\end{array}\right.$$
\par $\overline{\varphi}$ est surjectif car $\overline{p'}=\overline{p} \Rightarrow \varphi(p)=\varphi(p')$
\par Alors $\overline{\varphi}$ est un morphisme bijectif de $\Z/n_0\Z$ dans $G$
\par Donc $\Z/n_0\Z$ est isomorphe à $G$}
...
\begin{Rem}
Si $G$ est fini, tous les éléments de $G$ ont un ordre.
\end{Rem}
\Prop{Théorème de Lagrange}{Si $G$ est fini et $a\in G$, alors l'ordre de $a$ divise le cardinal de $G$.}
\Prop{}{Soit $G$ un groupe fini et $a\in G$\begin{itemize}
...
\end{itemize}}
\Pre{$\{n\in\N^*\vert a^n=e\}\subset \N$ et $\{n\in\N^*\vert a^n = e\}\neq \emptyset$
\par On peut donc noter $n_0 = \min \{n\in\N^*\vert a^n = e\}$
\par Il faut montrer que $\langle a\rangle =\{e, a, a^2,..., a^{n_0-1}\}$ et que $\mathrm{card}\langle a\rangle = n_0$
\par Pour $n\in\Z$, on peut écrire la division euclidienne de $n$ par $n_0$ :
\par $n = n_0q+r$ avec $0\leq r<n_0$
\par Alors $a^n = a^{n_0q}a^r = a^r$
\par Donc $\langle a\rangle = \{e, ..., a_{n_0-1}\}$
\par D'autre part, pour $n,p\in\Z$, avec $0\leq p<n<n_0$
\par Alors $a^{n-p}\neq e$ par définition de $n_0$
\par Donc $a^n\neq a^p$
\par Ce qui conclut : deux éléments de l'ensemble $\{e, a, a^2,..., a^{n_0-1}\}$ sont distincts.
\par Donc l'ordre de $a$ est bien $n_0$.
\par Pour l'autre propriété, on note $n_0= \mathrm{ordre}\langle a\rangle$
\par Soit $n\in\Z$, on fait la division euclidienne de $n$ par $n_0$, ce qui donne $n=n_0p+r$
\par Donc $a^n =  a^r$
\par Donc \begin{align*} a^n = e &\Leftrightarrow r = 0\\
&\Leftrightarrow n_0\vert n
\end{align*}}

\section{étude de Z/nZ}
On a vu que $(\Z/n\Z,+)$ est un groupe.
\par En fait, $(\Z/n\Z, +,\times)$ est un anneau commutatif.
...
\end{itemize}}
On a que les classes d'équivalences sont $\overline{x} = x+n\Z$
\par La relation d'équivalence est compatible avec la relation de congruence modulo $n$ parce que $x+n\Z+y+n\Z = x+y+n\Z$
\Def{Lois de composition interne}{On définit la loi additive par :
...
\Def{}{On note $\varphi(n) = \mathrm{Card}(\Z/n\Z)^\times = \mathrm{Card}\{\in\llbracket 0,n\rrbracket, p\wedge n = 1\}$
\par On appelle $\varphi$ l'indicatrice d'Euler.}
...
\end{itemize}}
\begin{Rem}
On voit donc que $\varphi(n)$ est le nombre de générateurs de $(\Z/n\Z, +)$
\end{Rem}
\Pre{Pour la première propriété :
\par Si $p\wedge n =1$ :
\par Soit $q\in\N^*$
\par \begin{align*}
q\overline{p} = 0 &\Leftrightarrow n\vert qp\\
&\Leftrightarrow n\vert q
\end{align*}
\par Donc l'ordre de $\overline{p}$ est $n$.
\par Donc $\overline{p}$ engendre $\Z/n\Z$
\par Réciproquement :
\par On suppose que $p\wedge n=d$ avec $d>1$
\par On pose donc $\begin{cases} p = dp' \\ n = dn'\end{cases}$ avec $p'\wedge n' = 1$
\par Alors $n'\overline{p} = \overline{n'dp'} = \overline{np'} =\overline{0}$
\par Donc l'ordre de $\overline{p}$ est strictement inférieur à $n$
\par D'où l'équivalence.}
\Pre{Pour la deuxième propriété :
\par Si $p\wedge n = 1$
\par D'après le théroème de Bézout, on a $u,v\in \Z$ tels que $pu+nv=1$
\par Donc $\overline{p}\overline{u} = \overline{1}$
\par Donc $\overline{p}$ est un inversible de $(\Z/n\Z,\times)$ et $\overline{p}^{-1}=\overline{u}$
\par Réciproquement :
\par On suppose que $p\wedge n=d$ avec $d>1$
\par On pose donc $\begin{cases} p = dp' \\ n = dn'\end{cases}$ avec $p'\wedge n' = 1$
\par Alors $n'\overline{p} = \overline{n'dp'} = \overline{np'} =\overline{0}$
\par On a donc $\overline{n'}\overline{p} = \overline{0}$ et $n'\neq 0$
\par Donc $\overline{p}$ n'est pas inversible.
\par D'où l'équivalence.}

\Prop{Corollaire}{$\Z/n\Z$ est un corps si, et seulement si, $n$ est premier.
\par Dans ce cas, on note $\mathbb{F}_n$ ce corps.}

\Thr{Euler-Fermat}{Soit $n\in\N,n\geq 2$ et $a\in(\Z/n\Z)^\times$
\par Alors :
\par $$a^{\varphi(n)}=\overline{1}$$}
\Thr{Euler-Fermat (autre formulation)}{Soit $n\in\N, n\geq 2$, alors :
\par $$\forall a\in\Z, a\wedge n =1 \Rightarrow a^{\varphi(n)}$$}
\Pre{Soit $\alpha$ l'ordre de $a$ dans $(\Z/n\Z)^\times$
\par $q$ divise $\varphi(n)$ (par Lagrange)
\par Donc $\varphi(n) = \alpha q$ (avec $\alpha\in\N)$
\par $a^{\varphi(n)}-a^{\alpha q} = 1^q - 1$}
\Thr{Petit théorème de Fermat}{Si $n$ est premier et $a\in\llbracket 1, n-1\llbracket$, alors
\par $$a^{n-1}\equiv 1(n)$$}
...
\Pre{On considère :
\par $$\varphi:\left\{\begin{array}{rcl}\Z & \to & \Z/n\Z\times \Z/p\Z \\ q & \mapsto & (\overline{q_n}, \overline{q_p})\end{array}\right.$$
\par $\varphi$ est un morphisme d'anneaux, et on a :
\par \begin{align*}
\ker\varphi &= \{q\in\Z\vert n\vert q\text{ et }p\vert q\}\\
&= \{q\in\Z\vert np\vert q\}\\
&=np\Z
\end{align*}
\par On peut donc considérer :
\par $$\overline{\varphi} :\left\{\begin{array}{rcl}\Z/np\Z & \to & \Z/n\Z \times \Z/p\Z \\ \overline{x_{np}} & \mapsto & (\overline{x_n},\overline{x_p})\end{array}\right.$$
\par $\overline{\varphi}$ est donc un morphisme d'anneaux injectif, puisque une version restreinte de $\varphi$ de manière à être injective.
\par Or $\mathrm{Card}(\Z/np\Z)=\mathrm{Card}(\Z/n\Z\times \Z/p\Z)$
\par Donc $\overline{\varphi}$ est bijective, d'où l'isomorphie des anneaux.
\par On remarque aussi que $\overline{\varphi}((\Z/np\Z)^\times) = (\Z/n\Z)^\times\times(\Z/p\Z)^\times$}
\begin{Rem}
On va expliciter le caractère surjectif de $\overline{\varphi}$
\par étant donné $(a,b)\in\Z^2$, on cherche $q$ tel que $\begin{cases} q\equiv b[n] \\ q\equiv a[n] \end{cases}$
\par Avec l'algorithme d'Euclide étendu, on peut trouver $(u,v)\in\Z^2$ tels que $nu+pv=1$
\par On a donc :
\par $\begin{cases} nu \equiv 1 [p] \\ nu \equiv 0 [n] \end{cases}$ donc $\begin{cases} anu \equiv a [p] \\ anu \equiv 0 [n] \end{cases}$
\par $\begin{cases} pv \equiv 1 [n] \\ pv \equiv 0 [p] \end{cases}$ donc $\begin{cases} bpv \equiv b [n] \\ bpv \equiv 0 [p] \end{cases}$
\par On peut donc prendre $q=anu+bpv$ qui est un élément de $\Z/np\Z$ tel que $\overline{\varphi}(q) = (a,b)$
\end{Rem}

\Prop{}{Si $n\wedge p = 1$, alors $\varphi(np) =\varphi(n)\varphi(p)$}
\Pre{Se fait par décomposition en produit de nombres premiers.}
\begin{Rem}
On peut étendre cette propriété par récurrence à des produits de nombres deux à deux premiers. On peut alors en déduire une manière de calculer $\varphi(n)$ de manière plus pratique :
\par Si $n = \prod\limits_{i=1}^rp_i^{v_{p_i}(n)}$ est la décomposition en nombres premiers de $n$
\par Alors $\varphi(n) =\prod\limits_{i=1}^r\varphi(p_i^{v_{p_i}(n)})$
\par Or pour $p$ premier on a que :
\par \begin{align*}
\varphi(p^\alpha) &= \mathrm{Card}\llbracket1,p^\alpha\llbracket - \mathrm{Card}\{pk, 1\leq k\leq p^{\alpha-1}\}\\
&=p^\alpha -p^{\alpha-1}\\
&=p^\alpha\left(1-\frac{1}{p}\right)
\end{align*}
\par Donc :
\par \begin{align*}
\varphi(n) &= \prod\limits_{i=1}^rp_i^{v_{p_i}(n)}\left(1-\frac{1}{p_i}\right)\\
&= n\prod\limits_{i=1}^r\left(1-\frac{1}{p_i}\right)
\end{align*}
\end{Rem}
Et c'est la fin du programme de spé.
